...
private MimeMessage createMessage(String to, String verificationCode) throws MessagingException, UnsupportedEncodingException {
    ...
    message.setSubject("Work Out Buddy 회원가입 인증 코드: "); //메일 제목
    ...
    message.setFrom(new InternetAddress(id, "wob_Admin")); //보내는 사람의 메일 주소, 보내는 사람 이름
   ...
   */
    public String sendAndSaveVerificationCode(MailConfirmDto mailConfirmDto) throws Exception {

        String to = mailConfirmDto.getEmail();
        // 인증코드 생성
    ...
    public boolean verifyEmail(MailVerifyDto mailVerifyDto) {
        ...
        String code = mailVerifyDto.getCode();
        String storedCode = emailVerificationCodes.get(email);

        if (storedCode != null && storedCode.equals(code)) {
            // 검증 성공 시, 사용한 코드는 삭제하여 재사용 방지
